...
* 🎭 **Dynamic Task Imbalance & Masking:** `vmap` usually crashes if tasks have different batch sizes. We engineered a robust **Masking & Padding engine** under the hood. You can now train on highly imbalanced tasks (`imbalanced_shot=True, support_shot=(min_shot,max_shot)`) without breaking vectorization!
